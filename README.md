...
1. Extracts the file ID from `/file/FILE_ID/...` or `/download/FILE_ID/...`.
2. Fetches the MediaFire landing page with a browser-like user agent.
3. Finds the real download anchor in the page:
...
- `fastget` currently resolves one MediaFire file per share URL. Folder-style
  MediaFire pages are not expanded into multiple files.
...
- MediaFire support depends on the current landing page exposing a
  `downloadButton` anchor with the signed CDN URL.
